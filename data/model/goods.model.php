...
class goodsModel extends Model {
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
        $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_commonid,goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, 'rand()', $limit, 'goods_commonid');
        ...
        if (!empty($goods_id_list)) {
            ...
            $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in', $tmp)), 'goods_commonid,goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, 'rand()', $limit);
        }
        foreach ($goods_commend_list as $i => $j) {
            $goods_common = Model('goods_common')->where(array('goods_commonid' => $j['goods_commonid']))->field('integral')->find();
            $goods_commend_list[$i]['integral'] = $goods_common['integral'];
        }
            ...
            if (!empty($goods_info['jjg_info'])) {
                ...
                if ($jjgFirstLevel && $jjgFirstLevel['mincost'] > 0) {
                    $goods_info['jjg_explain'] = sprintf(
                            '购满<em>&yen;%.2f</em>，再加<em>&yen;%.2f</em>，可换购商品', $jjgFirstLevel['mincost'], $jjgFirstLevel['plus']
                    );
                }
            }
