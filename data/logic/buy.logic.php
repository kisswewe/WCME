...
class buyLogic {
    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0) {
        ...
        list($store_cart_list, $store_goods_total, $store_integral_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
        ...
        $result['store_integral_total'] = $store_integral_total;
        //预定商品不使用任何优惠
        if (!$data['is_book']) {
...
//             $order_platform_store_ids[$k] = null;
    ...
    private function _createOrderStep3() {
        ...
        foreach ($goods_list as $key => $value) {
            ...
            $whe['goods_commonid'] = $result['goods_commonid'];
            $goods_common = Model('goods_common')->where($whe)->field('integral')->find();
            $integral_total += $goods_common['integral'] * $value['goods_num'];
        }
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        list($store_cart_list, $store_goods_total,$store_integral_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
        ...
        $this->_order_data['store_integral_total'] = $store_integral_total;
        $this->_order_data['store_final_order_total'] = $store_final_order_total;
        $this->_order_data['integral_total'] = $integral_total;
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
            ...
            $order['integral_amount'] = $store_integral_total[$store_id];
