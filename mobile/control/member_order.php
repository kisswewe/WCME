...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
        ...
        //wap订单列表商品积分
        foreach($new_order_group_list as $k => $i){
            foreach($i['order_list'] as $v => $j){
                foreach($j['extend_order_goods'] as $m => $n){
                    $result_goods_common = Model('goods_common')->where(array('goods_commonid' => $n['goods_id']))->find();
                    $new_order_group_list[$k]['order_list'][$v]['extend_order_goods'][$m]['integral'] = $result_goods_common['integral'];
                }
            }
        }
