...
class member_buyControl extends mobileMemberControl {
    ...
    public function buy_step1Op() {
        ...
        $buy_list['store_integral_total'] = $result['store_integral_total'];
    ...
    public function payOp() {
        ...
        foreach ($order_list as $key => $order_info) {
            if (!in_array($order_info['payment_code'],array('offline','chain'))) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) {
                    ...
                    $pay['pay_diff_integral'] += $order_info['integral_amount'];
                }
            }
        }
        ...
        $pay['pay_integral'] = ncPriceFormat($pay['pay_diff_integral']);
        unset($pay['pay_diff_integral']);
