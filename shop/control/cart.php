...
class cartControl extends BaseBuyControl {
    ...
    public function indexOp() {
        ...
        foreach($cart_list as $i=>$j){
            $goods_common = Model('goods_common')->where(array('goods_commonid' => $j['goods_commonid']))->field('integral')->find();
            $cart_list[$i]['integral']=$goods_common['integral'];
        }
        // 加价购活动
    ...
    //    print_r($_GET);exit;
        if (is_numeric($_GET['goods_id'])) {
            ...
            $goods_common = Model('goods_common')->where(array('goods_commonid' =>$goods_info['goods_commonid']))->field('integral')->find();
            //抢购
            ...
            $goods_info['integral'] = $goods_common['integral'];
            $quantity = 1;
        }
        ...
        $model_cart = Model('cart');
        $insert = $model_cart->addCart($goods_info, $save_type, $quantity,$goods_common['integral']);
    ...
    public function updateOp() {
        ...
        $goods_id=Model('goods')->where(array('goods_id' => $cart_info['goods_id']))->field('goods_commonid')->find();
        $goods_common = Model('goods_common')->where(array('goods_commonid' => $goods_id['goods_commonid']))->field('integral')->find();
        if ($cart_info['bl_id'] == '0') {
            ...
            if (intval($goods_info['goods_storage']) < $quantity) {
                ...
                $return['integral'] = $goods_common['integral'] * intval($goods_info['goods_storage']);
            ...
            foreach ($goods_list as $goods_info) {
                if ($quantity > $goods_info['goods_storage']) {
                    ...
                    $return['integral'] = $goods_common['integral'] * $quantity;
                    $model_cart->editCart(array('goods_num' => $goods_info['goods_storage']), array('cart_id' => $cart_id, 'buyer_id' => $_SESSION['member_id']));
                    exit(json_encode($return));
                    break;
                }
