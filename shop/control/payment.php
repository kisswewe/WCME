...
class paymentControl extends BaseHomeControl{
    ...
    public function real_orderOp(){
        ...
        if (empty($api_pay_amount)) {
            //支付完成扣除积分
            $buyer = Model('orders')->where(array('pay_sn'=>$order_pay_info['data']['pay_sn']))->select();
            $sum_integral_amount = 0;
            foreach($buyer as $item){
                $sum_integral_amount += $item['integral_amount'];
            }
            if($sum_integral_amount > 0 ){
                Model('member')->where(array('member_id'=>$buyer[0]['buyer_id']))->setDec('member_points',$sum_integral_amount);
            }

            //添加积分日志
            $param = array(
                'pl_memberid' => $buyer[0]['buyer_id'],
                'pl_membername' => $buyer[0]['buyer_name'],
                'pl_points' => -$sum_integral_amount,
                'pl_addtime' => time(),
                'pl_desc' => '订单' . $order_pay_info['data']['pay_sn'] .'购物消费',
                'pl_stage' => 'order'
            );
            Model('points')->addPointsLog($param);
    ...
    public function returnOp(){
        ...
        //扣除积分
        $buyer = Model('orders')->where(array('pay_sn'=>$out_trade_no))->select();
        $sum_integral_amount = 0;
        foreach($buyer as $item){
            $sum_integral_amount += $item['integral_amount'];
        }
