...
class show_storeControl extends BaseStoreControl {
    ...
    public function indexOp() {
        ...
        if (!$this->store_decoration_only) {
            ...
            foreach ($recommended_goods_list as $i => $j) {
                $goods_common = Model('goods_common')->where(array('goods_commonid' => $j['goods_commonid']))->field('integral')->find();
                $recommended_goods_list[$i]['integral'] = $goods_common['integral'];
            }
    ...
    private function _getGoodsList($condition, $order = 'goods_id desc') {
        ...
        foreach ($recommended_goods_list[1] as $i => $j) {
            $goods_common = Model('goods_common')->where(array('goods_commonid' => $j['goods_commonid']))->field('integral')->find();
            $recommended_goods_list[1][$i]['integral'] = $goods_common['integral'];
        }
