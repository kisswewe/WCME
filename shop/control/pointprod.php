...
//        $pointprod_list = $model_pointprod->getPointProdList($where, '*', $orderby,'',20);
            if(isset($_GET['points_min'])){
                if($_GET['points_min'] <= 0){
                    $where['integral'] = array('gt',0);
                }else{
                    if(isset($_GET['points_max'])){
                        if($_GET['points_max'] <= 0){
                            $where['integral'] = array('gt',0);
                        }else{
                            $where['integral'] = array('between',array($_GET['points_min'],$_GET['points_max']));
                        }
                    }else{
                        $where['integral'] = array('egt',$_GET['points_min']);
                    }
                }
            }else{
                if(isset($_GET['points_max'])){
                    if($_GET['points_max'] <= 0 ){
                        ...
                        $where['integral'] = array('between',array('0.01',$_GET['points_max']));
                    }
                }else{
                    $where['integral'] = array('gt',0);
                }
        ...
        $points = Model('goods_common')->where($where)->order($order)->select();
//        var_dump($points);exit;
        Tpl::output('pointprod_list',$points);
//        Tpl::output('show_page', $model_pointprod->showpage(5));
        Tpl::output('show_page');
