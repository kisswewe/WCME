...
class Sms {
    ...
    public function send($mobile,$content) {
       $hao_sms_type=C('hao_sms_type');
		...
		if($hao_sms_type==3)
		{
			return $this->mysend_genblue($mobile,$content);
		}
    }

	//创蓝文化短信接口
	private  function mysend_genblue($mobile,$content){
		/* ========== 国际短信 ========== */
		define('PLUGIN_ROOT', BASE_DATA_PATH . DS .'api/smsapi');
		require_once PLUGIN_ROOT . DS .'genblue' . DS . 'ChuanglanSMS.class.php';
		$sms=new ChuanglanSMS(C('hao_sms_zh'),C('hao_sms_pw'));
		// 发送手机号（国家区号+手机号码）
		$mobile = '0086'.$mobile;
		$result=$sms->sendInternational($mobile,$content);
