        ...
        $.ajax({//提交订单信息
            ...
            success:function(result){
                ...
                for (var k in result.datas.store_final_total_list) {
                    ...
                    //总积分
                    $('#storeTotalIntegral' + k).html(result.datas.store_integral_total[k]);
                    totals += parseFloat(result.datas.store_final_total_list[k]);
                    totals_integral += parseFloat(result.datas.store_integral_total[k]);
                    // 留言
                    message[k] = '';
                ...
                // 计算总积分
                if (totals_integral <= 0) {
                    totals_integral = 0;
                }
                // $('#totalIntegral,#onlineTotalIntegral').html(totals_integral.toFixed(2));
            }
        });
    }
